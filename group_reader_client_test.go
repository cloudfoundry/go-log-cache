...
func TestGrpcClientGroupRead(t *testing.T) {
	...
	if logCache.readReqs[0].EnvelopeType != rpc.EnvelopeTypes_LOG {
		t.Fatalf("expected EnvelopeType (%v) to equal %v", logCache.readReqs[0].EnvelopeType, rpc.EnvelopeTypes_LOG)
	}
}
